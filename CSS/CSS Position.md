...
```
/* top, right, bottom, left 순서 (margin/padding과 동일) */
...
**문서 흐름에서 제거**되어 페이지에서 공간도 차지하지 않게 되며 가장 가까운 **조상 요소 중 위치 지정 요소(`position` 프로퍼티가 `static`이 아닌 요소)가 있다면 해당 조상 요소의 `padding` 박스(정확히는 `padding`과 `border`의 경계)를 기준으로 배치**한다.
...
```
※ 단, 조상 요소 중에 transform, filter 등이 적용된 경우 뷰포트가 아닌 해당 조상이 컨테이닝 블록이 된다. -> 실무에서 fixed가 예상대로 동작 안 할 때 가장 흔한 원인.
...
**가장 가까이에 있는 스크롤되는 조상 요소, 혹은 표 관련 요소를 포함한 컨테이닝 블록(가장 가까운 블록 레벨 조상)을 기준으로** `top`, `right`, `bottom`, `left`의 값에 따라 오프셋(위치 차이)을 적용해 위치시킨다. 문서 흐름에서 제거되지 않는다.

추가로 **sticky는 반드시 `top`, `right`, `bottom`, `left` 중 하나 이상을 지정해야 동작하며** 오프셋 값 없이 선언 시 `relative`처럼 동작한다. 또 아래와 같은 경우 동작하지 않을 수 있다.

```
※ sticky 동작 조건
  1. 조상 요소에 overflow: hidden / auto / scroll이 설정되어 있으면 sticky가 동작하지 않을 수 있음
  2. 스크롤 컨테이너가 sticky 요소보다 충분히 커야 함 (sticky 요소가 부모 끝에 닿으면 그 자리에 고정됨)
```
...
`position`이 `absolute`인 경우 가장 가까운 조상 요소 중 `position`이 `static`이 아닌 요소가 있다면 해당 조상 요소의 `padding` 박스 기준, 그렇지 않다면 초기 컨테이닝 블록(루트 요소인 `<html>`의 컨테이닝 블록 -> 뷰포트)이 컨테이닝 블록이 됐었고 `fixed`의 경우는 항상 `viewport`를 컨테이닝 블록으로 위치가 결정됐었다.
...
- `contain` 프로퍼티 값이 `layout`, `paint`, `strict`, `paint`인 경우(e.g. contain: paint;)
- `backdrop-filter` 값이 `none` 이외의 값인 경우(e.g. backdrop-filter: blur(10px);)

**[MDN Identifying the containing block]**
...
요소의 z축 값을 설정하는 프로퍼티로 **`position` 프로퍼티가 `static` 이외인 요소, flex 아이템, grid 아이템에 적용되며** 요소 출력 우선순위를 결정한다(아래 쌓임 맥락을 생성하는 경우 참조).
...
- `offset-path`가 `none`이 아닌 요소 (CSS Motion Path)

- `@keyframes`에서 `opacity` 이외의 속성을 애니메이션 중인 요소는 일부 브라우저에서 쌓임 맥락 생성

- ~~`-webkit-overflow-scrolling`이 `touch`인 요소.~~ -> 재 거의 모든 브라우저에서 deprecated된 속성. 레거시 iOS Safari 대응용이므로, 현재(2026) 신규 코드에선 사용하지 않아야 한다.
...
- 각 쌓임 맥락은 형제 요소와 완전히 독립적이며 쌓임을 처리할 땐 자손 요소만 고려한다. 즉, A 요소 내부의 `z-index` 값에 따른 쌓임 맥락은 A 외부에 영향을 끼치지 않는다. 요소의 내용이 스택된 후 전체 요소는 부모 요소의 쌓임 맥락에 포함된다.
