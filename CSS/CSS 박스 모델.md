...
- 대부분의 블록 요소는 block, inline 요소를 포함할 수 있다.
  단, `<p>`처럼 내부에 블록 요소를 허용하지 않는 예외도 있다. 또한 `<li>`의 경우 블록처럼 동작하지만 실제 display 기본값은 list-item이다.
...
- padding, border의 top/bottom은 시각적으로 적용되나 주변 요소를 밀어내지 않는다.

- margin-top/bottom은 적용되지 않는다.

- 수평(left/right) padding, margin, border는 적용되며 인접 요소를 밀어낸다.

- 인라인 요소의 높이는 line-height로 지정한다.

- 요소 내에 block 레벨 요소를 포함할 수 없다.

- 요소 사이에 공백(엔터, 스페이스)이 있으면 텍스트 노드로 처리되어 약 4px의 공백이 생긴다(폰트 크기에 따라 다를 수 있음).

- ex) `<span>` `<a>` `<strong>` `<img>` `<br>` `<input>` `<select>` `<textarea>` `<button>`
  - `<img>`, `<input>` 등 **replaced element**는 인라인이지만 width/height 적용 등 블록적 특성을 일부 가진다(또한 `<input>`, `<select>`, `<textarea>`, `<button`>도 사실 브라우저에서 inline-block으로 렌더링되는 경우가 많다).
...
- 내부에 block, inline 레벨 요소를 포함할 수 있다(자기 자신은 인라인 컨텍스트에 배치된다).

- inline 레벨 요소처럼 뒤에 공백(엔터, 스페이스 등)이 있는 경우 텍스트 노드로 처리되어 약 4px의 공백이 생긴다(폰트 크기에 따라 다를 수 있음).
