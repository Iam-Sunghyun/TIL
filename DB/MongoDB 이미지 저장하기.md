...
- [Multer 모듈로 전송받은 파일 파싱하기](#multer-모듈로-전송받은-파일-파싱하기)
  - [Multer 사용 예제](#multer-사용-예제)
- [이미지 파일 Clodinary에 저장하기](#이미지-파일-clodinary에-저장하기)
  - [Cloudinary에 계정 등록하기](#cloudinary에-계정-등록하기)
  - [API 자격 증명(credential) 환경변수 설정하기](#api-자격-증명credential-환경변수-설정하기)
  ...
  - [Cloudinary에 이미지 업로드](#cloudinary에-이미지-업로드)
...
- [이미지 파일 URL MongoDB에 저장하기](#이미지-파일-url-mongodb에-저장하기)
  - [Mongoose 스키마에 이미지 필드 추가](#mongoose-스키마에-이미지-필드-추가)
  - [Multer 메서드 호출](#multer-메서드-호출)
...
우선 `Multer` 모듈에 `options`을 전달하여 객체를 생성한다. 만약 옵션 객체를 생략하면 파일이 메모리에 보관되고 디스크에 기록되지 않는다.

그 후 `upload.single(fieldname)`, `upload.array(fieldname)` 등 `Multer` API로 요청에 전송된 `multipart/form-data` 데이터를 파싱해 요청(`req`) 객체에 추가한다.

파싱된 폼 데이터는 텍스트 필드는 `req.body`에, 파일은 `req.file/req.files`에 저장된다(단일 파일의 경우 `req.file`에 저장되고, 여러 파일은 `req.files`에 저장된다).
...
>[
  {
    ...
    encoding: '7bit',
    mimetype: 'image/jpeg',
    path: 'https://res.cloudinary.com/dowpf7g5p/image/upload/v1672756165/CampInfo/cpuaqu0mkwqkendx3ick.jpg', // 모든 절차를 끝낸 시점에 확인했기 때문에 cloudinary에 연결되어있다.. 원래는 위에서 지정한 로컬 디렉토리 'uploads'에 저장됨. 
    size: 31572,
...
이런 경우 신용카드같은 결제를 위한 요소를 연결해야 하는데 만약 API 자격 증명을 앱에 직접 넣어 Git Hub에 push라도 하게 되면 다른 사람에게 노출될 수 있다(누군가 악용하여 예상치 못한 비용이 발생할 수도 있음).
따라서 Cloudinary뿐 아니라 다른 서비스를 사용하더라도 API 크리덴셜을 앱에 직접 넣지 않도록 주의할 것.
```

## API 자격 증명(credential) 환경변수 설정하기

자격 증명을 앱 내에 직접 삽입하지 않기 위해 `CLOUDINARY_URL` **환경변수 값을 설정**해준다. 대응되는 값들은 Cloudinary 웹 사이트 대쉬보드의 product environment credentials에서 확인할 수 있다.
...
이 모듈로 애플리케이션 코드가 아닌 `.env` 파일에 자격 증명을 저장하여 자격 증명을 찾거나 업데이트하기 쉬워지고, 또 버전 관리 시스템에 자격 증명 파일을 둘 필요가 없어져(환경변수로 참조) 깃허브 같은 공개 소스 저장소에서 오픈 소스 프로젝트를 운영할 때 자격 증명 보안을 걱정하지 않아도 된다.
...
그런 다음 `.env` 파일에 API 키, 암호와 같은 자격 증명을 다음과 같은 형식으로 저장해준다. 
...
`dotenv` 모듈을 로드하여 `config()` 메서드를 호출하면 현재 디렉토리에서 이름이 `.env`인 파일을 탐색한다. 그 후 `.env` 파일에 정의했던 변수들을 가져와 Node API `process.env`에 할당한다.

`console.log()`로 환경변수가 제대로 출력되는 것을 확인할 수 있다.
...
그 다음 절차는 `Multer`로 파싱한 파일을 Cloudinary에 좀 더 쉽게 업로드하기 위해 `Multer-Storage-Cloudinary`라고 하는 npm 모듈을 사용해 볼 것이다. 

업로드 절차는 다음과 같으며 `Multer`, `Cloudinary`, `Multer-Storage-Cloudinary` 세 모듈을 사용한다.

1. multer로 파싱한 파일 저장 위치를 cloudinary로 설정(`Multer-Storage-Cloudinary` 사용)
2. cloudinary에 저장된 파일 정보(참조 URL 포함)를 가져와 multer에 추가(req.file/req.files)
...
`Multer` API 호출 시 파일 전송 폼의 name을 인수로 전달해줘야 한다. 

아래 코드에선 `upload.array(fieldname)`을 호출하여 `name="image"` 폼으로 전송된 여러 개의 파일 정보를 `req.files`에 저장한다.
...
인수 `public_id`는 `cloudinary`에 저장된 `폴더/고유식별자` 형태의 파일명이다.
```
...
삭제 작업을 요약하면 우선 HTML 폼 체크박스로 사용자가 삭제할 이미지를 체크하여 요청 body의 `deleteImage[]` 프로퍼티의 요소로 전송받았다. 전송받은 데이터와 일치하는 캠핑장 도큐먼트의 image 필드 요소들을 삭제한 후, `cloudinary` API를 사용해 파일 저장소인 `cloudinary`에서도 삭제해줌으로서 완전히 삭제해주었다.
