...
- [왜 일반적으로 CSS `<link>` 태그를 `<head></head>` 태그 사이에 위치시키고, JS `<script>` 태그를 `</body>` 직전에 위치시키는 것이 좋은 방법인가? 다른 예외적인 상황은?](#왜-일반적으로-css-link-태그를-headhead-태그-사이에-위치시키고-js-script-태그를-body-직전에-위치시키는-것이-좋은-방법인가-다른-예외적인-상황은)
  ...
  - [예외적인 상황들](#예외적인-상황들)
- [이미지 태그에 `srcset` 속성을 사용하는 이유는 무엇인가? 이 속성의 컨텐츠를 실행할 때 브라우저의 프로세스는?](#이미지-태그에-srcset-속성을-사용하는-이유는-무엇인가-이-속성의-컨텐츠를-실행할-때-브라우저의-프로세스는)
- [웹 접근성이란?](#웹-접근성이란)
...
# 이미지 태그에 `srcset` 속성을 사용하는 이유는 무엇인가? 이 속성의 컨텐츠를 실행할 때 브라우저의 프로세스는?
