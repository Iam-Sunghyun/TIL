...
- [HTML 폼(Form)](#html-폼form)
  - [예시](#예시)
- [자주 사용하는 HTML 폼 요소](#자주-사용하는-html-폼-요소)
  - [`<input>`](#input)
    - [속성 몇가지](#속성-몇가지)
  - [` <label>`](#-label)
  - [체크박스, 라디오 버튼](#체크박스-라디오-버튼)
  - [`<select>`](#select)
  - [`<input type="range">`](#input-typerange)
  - [`<textarea>`](#textarea)
  - [`<button>`](#button)
    - [구글 검색 이용해보기](#구글-검색-이용해보기)
- [HTML 폼 유효성 검사(validation)](#html-폼-유효성-검사validation)
  - [유효성 검사 필요성?](#유효성-검사-필요성)
    - [\[웹 사이트 보안\]](#웹-사이트-보안)
  - [클라이언트 측 유효성 검사](#클라이언트-측-유효성-검사)
    - [\[클라이언트 측 유효성 검사\]](#클라이언트-측-유효성-검사-1)
...
- **올바른 형식의 데이터를 얻기 위함** - 사용자 데이터가 잘못된 형식으로 저장되거나 올바르지 않거나 완전히 생략되면 애플리케이션이 제대로 작동하지 않을 수 있다.
- **사용자 정보 보안을 위해** - 보안 암호를 사용하여 사용자 계정을 보호한다.
...
클라이언트 측 유효성 검사는 크게 두 가지 유형이 있다.

- **기본 양식 유효성 검사**
- **자바스크립트 유효성 검사**

기본 양식 유효성 검사는 별도의 자바스크립트 코드 없이 사용할 수 있는 HTML 기본 validation 속성들을 사용하여 유효성을 검사하는 방법이다.
