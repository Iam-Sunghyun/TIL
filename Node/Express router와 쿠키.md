...
서명된 쿠키란 쿠키가 변조 되지 않았음을 확인하기 위한 방법으로 편지를 밀랍으로 봉인하듯 `cookieParser(secret, options)`의 첫 번째 인수로 지정한 `secret`값을 쿠키의 서명으로 사용하며 요청 헤더의 서명된 쿠키를 파싱한다(지정하지 않으면 파싱하지 않음).
