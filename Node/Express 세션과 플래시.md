...
그 이유는 `MemoryStore`는 웹 서버의 메모리에 저장되는 것이기 때문에 서버를 재시작 하거나 브라우저를 종료하면 데이터가 사라진다. 또한 여러 대의 서버 상에서(로드 밸런싱)의 Session data 공유도 `MemoryStore`에서는 불가능한데 이것은 특정 서버의 메모리에만 사용자 정보가 저장되어있는 경우 다른 서버로 응답할 때 사용자 정보 조회할 수 없게되는 것을 의미한다.
