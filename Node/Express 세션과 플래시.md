...
클라이언트가 웹 서버에 연결되어 있는 상태(사용자가 웹 사이트에 접속하여 종료하기 전까지 상태, 기간) 혹은 연결되어 있는 동안 발생하는 사용자 정보를 서버 측에 일시적으로 저장하는 것을 세션이라 한다(DB처럼 영구적인 것이 아니므로 브라우저를 종료하거나 서버를 재시작하면 삭제된다).

## 세션 사용 이유?

쿠키의 경우 보통 4KB의 용량 제한과 20개의 개수 제한이 있어서 사용자 정보가 매우 많을 경우 모두 담아내지 못할 수 있다. 또한 클라이언트측에 저장되어 누구나 쉽게 열어보고 조작할 수 있고, 중요한 정보를 쿠키에 직접 담을 경우 요청 전송 중 가로채질 수 있는 등 보안적인 부분에서 서버 측에 저장하는 것보다 안전하지 않다(쿠키의 보안적인 문제는 Express router와 쿠키.md 참고자료 참고). 
...
그 이유는 `MemoryStore`는 웹 서버의 메모리에 저장되는 것이기 때문에 서버를 재시작 하거나 브라우저를 종료하면 데이터가 사라진다. 또한 여러 대의 서버 상에서의 Session data 공유도 `MemoryStore`에서는 불가능한데 이것은 특정 서버의 메모리에만 사용자 정보가 저장되어있는 경우 다른 서버로 응답할 때 사용자 정보 조회할 수 없게되는 것을 의미한다.

따라서 실제 production 환경에서는 세션 데이터를 Redis, MongoDB와 같은 DB를 사용하여 영속적으로 관리하는 것이 일반적이라고 한다.

`MemoryStore` 이외에도 Redis, mongo 등 호환되는 여러가지 세션 저장소가 있다.
