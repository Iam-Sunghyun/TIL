...
- [모듈(module)?](#모듈module)
  - [모듈화 필요성](#모듈화-필요성)
- [Nodejs 모듈 시스템 결정](#nodejs-모듈-시스템-결정)
  - [ES 모듈(자바스크립트 내장 모듈)](#es-모듈자바스크립트-내장-모듈)
    - [import, export](#import-export)
    - [export default](#export-default)
  - [Commonjs(CJS)](#commonjscjs)
    - [module.exports, exports?](#moduleexports-exports)
  - [디렉토리로 파일 불러오기](#디렉토리로-파일-불러오기)
- [자바스크립트 모듈 특징](#자바스크립트-모듈-특징)
- [NPM(Node Package Manger)](#npmnode-package-manger)
  - [`node_modules`](#node_modules)
  - [`package.json`](#packagejson)
  - [`package-lock.json`](#package-lockjson)
    - [`node_modules`를 직접 내려받으면 되지 않는가?](#node_modules를-직접-내려받으면-되지-않는가)
  - [패키지 지역 설치, 전역 설치](#패키지-지역-설치-전역-설치)
    - [지역 설치(local install)](#지역-설치local-install)
    - [전역 설치(global install)](#전역-설치global-install)
  
# 모듈(module)?

모듈(module)이란 프로그램을 구성하는 개별적 요소로, 재사용 가능한 코드 조각(파일)들을 말한다.
...
자바스크립트는 웹페이지에 있어서 보조적인 기능을 수행하기 위해 한정적인 용도로 만들어진 태생적 한계로 다른 언어에 비해 부족한 부분이 있는 것이 사실이다. 그 대표적인 것이 모듈 시스템을 지원하지 않았었다는 것.
...
Node.js에는 CommonJS, ECMAScript 모듈(ESM) 두 가지 모듈 시스템이 있다.

각 모듈을 활성화하는 방법과 사용 방식의 차이를 알아본다.
...
Node.js의 기본 모듈화 시스템이다. 동기 방식으로 동작하기 때문에 서버 사이드에서 사용하기 좋다.
...
--------------------------------------------
// square.js
// exports에 프로퍼티 없이 직접 할당하면 지역변수가 되어버린다. 따라서 module.exports에 직접 할당할 것.
...
조심해야 할 것은 클래스같은 하나의 객체, 값을 exports하고 싶을때 `module.exports`에 할당하지 않고 `exports`에 할당하게 되면 `module.exports`에 대한 참조 값이 덮어씌워져 모듈화 되지 않고 exports란 이름의 지역변수가 되어 버린다.
...
예를 들면 require('/폴더1')로 파일이 아닌 디렉토리를 로드했을 때 폴더1의 index.js파일이 export한 내용을 로드하게 된다.
