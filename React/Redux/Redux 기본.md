...
- [React-redux](#react-redux)
  - [리덕스 `store` 생성](#리덕스-store-생성)
  - [`store` 제공(Provide) 하기](#store-제공provide-하기)
...
<!-- 리덕스는 MVC 패턴의 양방향 데이터 흐름으로 인한 복잡하고 예측하기 힘든 

mvc -> flux -> redux
리덕스를 쓰는 이유, 만들어진 이유 내용 추가 필
-->
...
<!-- Context API 대신 Redux 사용 이유, 차이점 내용 수정 필 -->
...
그렇다고 하나의 `Context`에 모든 것을 담기엔 너무 복잡해지고, 또 잦은 업데이트로 인해 불필요한 렌더링도 발생하게 된다(`Context`의 일부만 업데이트 해도 사용하는 모든 컴포넌트가 리렌더링되는 성질 때문).

<!-- React Redux는 내부적으로 많은 성능 최적화를 구현하므로 자신의 컴포넌트가 실제로 필요할 때만 다시 렌더링됩니다. -->
...
`React-redux`는 `redux`의 공식 리액트 바인딩으로 리액트 컴포넌트에서 리덕스 `store` 데이터를 읽거나 `dispatch` 하는 등 `redux` 기능을 쉽게 사용할 수 있게 만들어주는 공식 도구이다.  

`React-redux`를 통해 리액트 앱에서 `redux`를 사용해본다.

공식 문서에서 권장하는 시작법은 `Create React App` 전용 리덕스+자바스크립트 혹은 리덕스+타입스크립트 템플릿을 사용하는 것이다.
...
혹은 `CRA`로 이미 생성한 프로젝트의 경우 다음과 같이 `react-redux`만 다운로드 받아준다.
...
`react-redux`에서 제공하는 `<Provider />` 컴포넌트로 리액트 앱에서 `store`를 제공할 수 있다.

`<Provider />` 컴포넌트의 사용 방식은 `Context API`와 비슷하다.

상태를 공유할 하위 컴포넌트들을 포함하는 부모 컴포넌트를 감싸주어 `store` `prop`으로 전달해주면 된다. 
...
`useSelector()`를 사용하면 자동으로 `store`에 `subscribe` 되며 `store`의 데이터가 변경될 때마다 컴포넌트가 렌더링된다. 또 컴포넌트가 언마운트 되면 자동으로 `subscribe`가 해제된다.

`useSelector()`의 인수로 전달하는 콜백 함수(선택기 함수라고 함)는 컴포넌트가 렌더링될 때마다 인수로 `store`의 상태를 전달받아 호출되고, 선택기 함수의 반환 값이 곧 `useSelector()`의 반환 값이 된다. 

선택기 함수는 `action`이 `dispatch` 될 때마다 호출되는데 이때 선택기 함수의 이전 반환 값과 `dispatch` 후 새 반환 값을 비교하여 값이 다른 경우 컴포넌트를 렌더링하고, 같을 경우 렌더링하지 않는다(따라서 상태가 객체인 경우 불변 데이터처럼 새 객체로 교체해줘야 변경을 추적할 수 있다 -> `reducer`가 순수해야 하는 이유).
