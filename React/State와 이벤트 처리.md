...
정리하면 **`state`란 컴포넌트 자기 자신 안에서 생성되고 제어되는 데이터로 렌더링 간에 유지되며 값 변경 시 렌더링을 트리거하여 반응성을 추가하는데 사용되는 데이터이다**(`props`와 달리 변경이 가능한 값이다).
...
제어 컴포넌트를 사용하면 입력될 때마다 state가 업데이트되고, 또 요소에 반영시켜 즉각적인 상호작용 가능하다는 장점이 있다.

예를들면 입력을 대문자로 받아야 하는 경우 사용자 입력 알파벳을 즉시 대문자로 바꿔서 form에 재출력하는 것과 같이 즉각적인 유효성 검사로 입력을 제한하거나 요소를 다르게 출력 등 유용하게 사용할 수 있다.

하지만 매 입력마다 state를 갱신하고, 렌더링이 일어나기 때문에 불필요한 렌더링이 발생할 수 있다.
...
React 없이 DOM을 통해 입력 값을 제어하는 경우, 혹은 useRef()로 특정 DOM 요소를 참조하여 값을 가져오거나 설정하는 것도 비제어 컴포넌트라 할 수 있다.
