...
제어 컴포넌트란
제어 컴포넌트 - 사용자가 입력을 state로 저장하고 출력도 state로 결정되는 컴포넌트. -> 입력될 때마다 set으로 state 업데이트. 즉각적인 상호작용 가능 ex) 사용자 입력 알파벳을 대문자로 바꿔서 form에 재출력, 즉각적인 유효성 검사로 입력을 제한하거나 요소를 다르게 출력 등
