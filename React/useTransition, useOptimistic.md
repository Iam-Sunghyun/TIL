...
- [`useTransition` 훅이란?](#usetransition-훅이란)
...
`useTransition`은 **상태 업데이트 함수의 실행 우선순위를 낮추어 업데이트를 지연 시킴으로 UI를 더 반응성 있게 만드는 역할**을 한다.
...
리액트 18v에서 비동기 렌더링 방식이 적용됨과 동시에 **`useTransition`의 도입으로 특정 상태 업데이트를 나중에 처리해도 되는 작업(non-urgent update)으로 만들 수 있다.** 즉, 사용자 입력과 같이 긴급한 업데이트가 아닌 상태 업데이트(예: 데이터 로딩 후 컴포넌트 렌더링)의 우선순위를 낮추어(백그라운드 예약) 후 순위로 처리할 수 있게 해준다.

만약 긴급한 업데이트가 발생하는 경우, 리액트는 예약됐거나 실행 중인 백그라운드 렌더링을 중단(취소)하고 새로운 긴급한 업데이트를 먼저 처리한다. 예를 들어 사용자가 새로운 문자를 입력하는 경우, 혹은 특정 탭을 클릭 했다가 마음이 바뀌어 다른 탭을 클릭하면 첫 번째 렌더링이 완료될 때 까지 기다릴 필요 없이 다른 탭을 클릭할 수 있다.
...
`useTransition`은 상태 업데이트(transition)이 진행 중 인지 알 수 있는 `isPending`과 상태 업데이트를 transition으로 표시할 수 있게 해주는 `startTransition` 함수를 반환 한다. 상태 업데이트 로직을 `startTransition` 함수에 전달하여 사용하며 전달되는 함수를 `Actions`라고 한다.

이때 **`startTransition`에 전달되는 `action`은 동기식이어야 한다.** `startTransition`를 호출하면 리액트는 전달한 함수를 즉시 실행하고, 실행하는 동안 발생하는 모든 `state` 업데이트를 transition으로 표시한다.
...
3. 입력 이벤트가 계속 발생하면 React는 기존의 transition 작업을 취소하고 새로 예약

4. 사용자가 멈추면 → transition 업데이트 실행됨

결과적으로, 입력 지연 없이 부드러운 UI 가 유지된다.

이때 `handleChange` 함수 실행 시 `startTransition`에 전달한 함수 또한 바로 실행되며 전달 된 콜백 자체는 지연되지 않는다. 즉, 콜백 함수는 즉시 실행되며 함수 내부의 상태 업데이트(`set` 함수)만 지연 된다.

`action` 함수 내에서 `await`된 비동기 호출은 transition에 포함되지만 `await` 이후에 상태 업데이트를 위해 `set` 함수를 연달아 호출 하는 경우 현재로서는(19.2v) 추가적으로 `startTransition`으로 감싸줘야 transition 업데이트로 분류 된다(향후 수정될 예정이라 함).
...
`useOptimistic`은 React 19v에서 도입된 훅으로, **비동기 작업이 진행 중일 때 응답을 기다리지 않고 미리 결과 상태를 반환하는** 훅이다(실패 시 원래 상태로 되돌아 간다).

`useOptimistic`을 사용하면 **사용자의 작업이 성공할 것이라고 가정하고 결과 상태를 미리 받아** 서버의 응답을 기다리는 동안에 UI를 미리 갱신하는 식으로 사용자 경험을 즉각적이고 매끄럽게 만들어줄 수 있다.

`useOptimistic` 훅은 '현재 상태'와, '작업의 입력을 매개변수로 받는 함수'를 매개변수로 받고, 결과 상태와 낙관적 업데이트 시(비동기 작업 시작 시) 호출되는 함수를 반환한다.
...
대표적 예시로 메시지 앱에서 사용자가 메시지를 입력하고 전송 버튼을 누르면, `useOptimistic`을 통해 메시지가 서버로 전송되기 전에 즉시 목록에 나타난다. 혹은 새로운 메시지를 전송할 때 "전송 중..."과 같은 상태를 보여주다가, 서버 응답이 완료되면 최종 상태로 업데이트하는 것이 있다.
