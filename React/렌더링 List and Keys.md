...
**리액트에서 키(key)는 컴포넌트를 구분하기 위한 고유한 식별자**로 기존에 있던(변경되지 않은) 컴포넌트를 식별하기 위해 사용한다.

키는 **배열에 담겨 반환되는 컴포넌트에서 중요하게 사용된다.** 키 값은 컴포넌트가 담긴 배열 내에서 고유한 값이어야 하며 서로 다른 배열에서 같은 키 값은 상관없다.
...
**키를 사용하면 배열 요소가 변경되었을 때(컴포넌트 추가, 삭제, 정렬 등) 변경되지 않은 컴포넌트를 식별하고 불필요한 렌더링을 막는다(렌더링이 필요한 컴포넌트만 렌더링). 이것은 재조정(Reconciliation) 시 변경 부분을 올바르게 추적할 수 있게 하여 불필요한 DOM 변경이 발생하지 않도록 해주므로 반드시 사용할 것은 권장**하고 있다. 
...
+추가로 확인해본 결과 **키는 컴포넌트가 배열에 담겨져 반환되는 경우 의미가 있었다.** 즉, 배열로 반환되는 경우가 아니라면 형제 요소들 맨 앞에 `<li>`를 추가했을때 나머지 항목들이 렌더링(함수 컴포넌트 호출)은 일어났으나 실제 DOM에서 새롭게 생성되지 않았다(React가 변경된 컴포넌트를 잘 식별함).

```
컴포넌트 렌더링 간에 특정 컴포넌트의 key 값을 의도적으로 변경하여 해당 엘리먼트를 재생성하게 만들 수 있다. 이때 기존 DOM 노드가 파괴 후 새롭게 생성되는데 해당 요소의 state를 리셋하기 위한 용도로 사용된다. 
```
