...
키는 어떤 데이터의 변경(추가, 제거 등)을 구별하기 위해 사용되며 배열 안의 컴포넌트라면 해당 배열 내에서만 고유한 값이면 된다. 즉, 다른 배열에서 같은 키 값은 상관없다.

<!-- 추가로 **키는 배열의 맥락에서만 의미가 있다.** -->

## 리스트에서 key를 사용해야하는 이유?
<!-- 중요 -->
React에서 컴포넌트의 상태가 변경되었을 때 React는 새로운 가상 DOM을 생성하고 diffing 알고리즘을 통해 이전의 DOM과 비교하여 변경 사항을 확인하고 변경된 부분만 실제 DOM에서 업데이트한다. 

**키를 사용하면 React가 리스트 요소가 변경되었을 때 변경 부분을 올바르게 추적할 수 있게 하여 불필요한 렌더링을 막을 수 있기 때문에 반드시 사용할 것은 권장**하고 있다(재조정(Reconciliation) 시 변경되지 않아도 될 엘리먼트들을 제대로 추적할 수 있음). 
