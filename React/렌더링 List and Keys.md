...
<!-- 중요 -->
키를 사용하는 이유는 React가 리스트 요소의 위치나 데이터가 변경되었을 때 올바르게 추적할 수 있게하여 업데이트 시 불필요한 렌더링을 막을 수 있기 때문이다(반드시 사용할 것은 권장하고 있다). 

React에서 컴포넌트의 상태가 변경되었을 때 React는 새로운 가상 DOM을 생성하고 이전의 DOM과 비교하여(diffing 알고리즘) 변경 사항을 확인하고 변경된 부분만을 실제 DOM에서 업데이트한다. 
