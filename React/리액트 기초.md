...
리액트는 **컴포넌트 기반**으로 동작하는데 컴포넌트란 리액트 앱에서 재사용 가능한 빌딩 블록으로, 화면에 출력될 UI를 여러 조각으로 나누어 놓은 것이라 생각하면 된다(함수 컴포넌트와 클래스 컴포넌트가 있다). 예를들면 네비게이션 바, 게시물 블록, 버튼과 같은 것들 모두 하나의 컴포넌트이며 필요에 따라 더 세부적으로 분해할 수도 있다.
...
`JSX`란 **자바스크립트를 확장한 문법**으로 `Javascript XML`이라고도 한다. 아래 `App.js`는 `JSX`이며 자바스크립트 내에 `HTML`을 포함하고 있다(실제로 `HTML`은 아니다). 이것이 리액트 요소를 반환하는 함수 컴포넌트이다.
...
컴포넌트 합성을 위해선 `props`를 사용해야 한다. 위에서 `props`에는 컴포넌트 요소의 어트리뷰트와 자식 요소들이 객체에 담겨져 전달된다고 했었다.
