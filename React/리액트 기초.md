...
컴포넌트란 리액트 앱에서 재사용 가능한 빌딩 블록으로, 화면에 출력될 UI를 여러 조각으로 나누어 놓은 것이라 생각하면 된다(함수 컴포넌트와 클래스 컴포넌트가 있다). 
...
`JSX`란 **자바스크립트를 확장한 문법**으로 `Javascript XML`이라고도 한다. 

아래 `App.js`가 반환하는 내용이 `JSX`이며 자바스크립트 내에 `HTML`과 비슷한 마크업을 포함하고 있다(실제로 `HTML`은 아니다). 이것이 리액트 요소를 반환하는 함수 컴포넌트이다.
