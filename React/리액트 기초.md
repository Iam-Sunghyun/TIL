...
자바스크립트 파일 내에 `JSX` 구문이 브라우저에서 동작할 수 있는 것은 `JSX` 구문이 `Babel`로 인해 자바스크립트로 변환되기 때문(`JSX`는 실제로는 자바스크립트 코드인 `React.createElement(component, props, ...children)` 함수에 대한 문법적 설탕일 뿐이다). 

`Create React App`로 생성된 프로젝트에는 `Babel`이 포함되어 있으며 이것으로 코드를 변환한다.
...
`props`는 읽기 전용(불변 값처럼 사용해야 함)이며 `state`는 변경이 가능하다.
