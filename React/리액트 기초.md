...
- [리액트 핵심 구문 `JSX`](#리액트-핵심-구문-jsx)
  - [`JSX`가 반환하는 요소가 하나의 요소에 담겨져 있어야 하는 이유](#jsx가-반환하는-요소가-하나의-요소에-담겨져-있어야-하는-이유)
  - [컴포넌트 생성 및 사용해보기](#컴포넌트-생성-및-사용해보기)
- [import로 CSS 스타일 추가하기](#import로-css-스타일-추가하기)
- [props와 state](#props와-state)
- [컴포넌트 합성(composition)](#컴포넌트-합성composition)
...
리액트는 **컴포넌트 기반**으로 동작하는데 컴포넌트란 리액트 앱에서 재사용 가능한 빌딩 블록으로, 화면에 출력될 UI를 여러 조각으로 나누어 놓은 것이라 생각하면 된다(함수형 컴포넌트와 클래스형 컴포넌트가 있다). 예를들면 네비게이션 바, 게시물 블록, 버튼과 같은 것들 모두 하나의 컴포넌트이며 필요에 따라 더 세부적으로 분해할 수도 있다.
...
`JSX`란 **자바스크립트를 확장한 문법**으로 `Javascript XML`이라고도 한다. 아래 `App.js`는 `JSX`이며 자바스크립트 내에 `HTML`을 포함하고 있다(실제로 `HTML`은 아니다). 이것이 리액트 요소를 반환하는 함수형 컴포넌트이다.
...
또한 컴포넌트 안에서 렌더링되는 `JSX`는 하나의 루트 요소로만 이루어져 있어야한다. 
...
```
## `JSX`가 반환하는 요소가 하나의 요소에 담겨져 있어야 하는 이유
...
이말은 `JSX`는 브라우저에서 실행되기 전 자바스크립트로 변환되는 과정을 거치는데 여러 요소를 반환하는 `JSX` 컴포넌트를 자바스크립트로 변환한 예시를 살펴보면 아래와 같다(최종 변환 형태는 아님).
...
CSS 스타일을 컴포넌트에 추가하려면 적용하고자 하는 컴포넌트에 `import` 해준다(꼭 스타일을 적용하려는 컴포넌트 파일이 아니어도 해당 컴포넌트의 상위 컴포넌트면 된다. 그러나 혼란을 줄이기 위해 적용하고자 하는 컴포넌트 파일에 `import` 해주는 게 좋을 듯).
...
우선 `props`는 다른 컴포넌트로부터 전달 받는 데이터이고, `state`는 컴포넌트 자기자신 내에서 생성되고 관리되는 데이터이다.

`props`는 읽기 전용(불변 값)이며 `state`는 변경이 가능하다.
...
컴포넌트 합성을 위해선 `props`를 사용하여 줘야하는데 위에서 `props`에는 사용자 정의 컴포넌트 요소의 어트리뷰트와 자식 요소들이 객체에 담겨져 전달된다고 했었다.

전달된 자식 요소들은 **`props.children` 프로퍼티를 통해 참조할 수 있으며** 래퍼 컴포넌트로서 자식 요소들을 포함시키기 위해서는 아래와 같이 `div` 내부에 `props.children`를 전달해줘야 한다.  

※ 아래 코드에서 주의할 점은 사용자 정의 컴포넌트 요소에 `className='expense-item'`으로 지정한 CSS 스타일을 적용하려면 호출되는 컴포넌트의 리액트 요소에 클래스 이름을 직접 전달해줘야 한다는 것. 그렇지 않으면 사용자 정의 컴포넌트 요소에 지정한 `className='expense-item'`의 CSS 스타일이 적용되지 않는다.
...
function Card(props) {
  const classes = 'card ' + props.className;  // 적용하고자 하는 CSS 클래스도 같이 전달해줘야 한다
...
function ExpenseItem(props) {
  ...
  return (
    <Card className='expense-item'>   // 이대로만 작성한다면 expense-item 클래스 스타일은 적용되지 않는다. 반드시 Card 컴포넌트 내부 요소에 props로 전달해줘야 한다
      <div className='expense-item__description'>
        <ExpenseDate date={props.date} />
        <h2>{props.title}</h2>
      </div>
