...
- [`<form>`에서의 서버 함수](#form에서의-서버-함수)
  - [폼 `action` 함수의 반환 값으로 `state` 업데이트 하기(`useActionState`)](#폼-action-함수의-반환-값으로-state-업데이트-하기useactionstate)
...
서버 함수는 **서버 컴포넌트 혹은 서버 파일 내에서만 정의할 수 있으며 클라이언트 컴포넌트의 `props`를 통해 전달하거나**(일반 함수는 서버 컴포넌트에서 클라이언트 컴포넌트로 전달할 수 없지만 서버 함수는 참조를 전달하는 것이기 때문에 가능하다) **별도의 파일에 정의하여 서버, 클라이언트 상관 없이 `import` 하여 전달할 수 있다(권장).**

그렇게 되면 리액트는 자동으로 서버 함수에 대한 참조를 생성하고 해당 참조 값을 클라이언트 컴포넌트에 전달한다. **서버 함수는 클라이언트에서 함수 참조 ID와 함께 네트워크 요청을 거쳐 서버에서 호출되는 것인데, 이때 전달된 모든 인수의 직렬화 된 사본을 포함한 네트워크 요청을 서버로 전송한다. 따라서 서버 함수에 전달되는 모든 인수는 직렬화 가능해야 한다.**
...
위 코드는 리액트 공식 홈페이지 예시로 `React`가 `EmptyNote` 서버 컴포넌트를 렌더링할 때, `createNoteAction` 함수에 대한 참조를 생성하고, 그 참조를 `Button` 클라이언트 컴포넌트에 전달한다. 버튼이 클릭 되면, `React`는 제공된 참조의 함수(`createNoteAction`)를 실행하라는 **`POST` 요청**을 서버로 보낸다.
...
`action`에 URL을 전달하면, 폼은 HTML 폼 요소처럼 동작하며 서버 함수를 전달하면, 폼 `submit` 시 리액트는 폼을 통해 제출된 `formData`를 직렬화 하여 `POST` 요청으로 서버에 함수 호출을 요청한다. **이때 `<form action>`에 전달 된 서버 함수는 자동으로 `startTransition` 내에서 호출된다.**
...
## 폼 `action` 함수의 반환 값으로 `state` 업데이트 하기(`useActionState`)
...
만약 서버 함수가 서버 mutation이 없고, `<form action>` 혹은 `<input type="image or submit">`, `<button>`의 `formaction` `prop`으로 호출되는 것이 아닌 경우라면, `startTransition` 안에서 호출해야 한다. 다만 이건 강제 되는 규칙은 아니며, 필수에 가까운 권장 패턴이다.
...
| ----------------------------------------------------------------------------------------------------- | ------------------------------------------- | ----------------------------------------------------- |
| **Form Action** (`<form action={action}>`, `<input image, submit formaction>`, `<button formaction>`) | HTML form 제출 시 자동으로 서버 액션 호출   | ❌ 필요 없음 (React가 내부적으로 transition 처리함)   |
