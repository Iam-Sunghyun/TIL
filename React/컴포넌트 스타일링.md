...
CSS Module 파일이 import되면 CSS 파일에 선택자로 지정한 이름(클래스 혹은 id)이 `[filename]_[classname]__[hash]` 형태의 고유한 이름으로 자동 변환되어 중복을 방지한다(**서로 다른 CSS 파일 내에 동일한 클래스, id 선택자 모두 고유한 별개의 값이 된다**).

```
vite로 생성한 프로젝트에서도 css module이 지원되는데 선택자가 변환되는 기본 형식이 약간 다르다. -> _선택자_고유값_숫자
```
...
CSS Module을 사용하면 별다른 네이밍 규칙 없이도 CSS 클래스 중복을 방지할 수 있다. 추가로 **특정 컴포넌트에만 스타일을 적용하기 위해선 클래스나 아이디 선택자가 아닌 일반 선택자(ex) ul, div ...)를 사용하지 않도록 주의해야 한다.**
