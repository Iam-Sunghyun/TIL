...
- [CSS Module](#css-module)
  - [CSS Module 동적 스타일 적용하기](#css-module-동적-스타일-적용하기)
...
이러한 특징의 문제는 중복된 선택자로 특정 컴포넌트에 import된 CSS 파일의 스타일이 애플리케이션 내의 다른 컴포넌트의 엘리먼트에도 적용되어 버릴 수 있다는 것이다.

이 문제를 피할 수 있는 일반적인 2가지 방법이 있는데 다음과 같다( 아래의 라이브러리들은 특정 컴포넌트에 적용할 스타일이 다른 컴포넌트에 영향을 끼치지 않게 하기 위한 도구들이다).
...
`Styled-components`는 특정 컴포넌트에만 스타일을 적용하여 컴포넌트를 생성해주는 npm 패키지로, 자바스크립트 안에 CSS를 작성하는 방식으로 컴포넌트에 스타일을 적용한다. 
...
컴포넌트 생성 메서드에 태그드 템플릿 리터럴(Tagged Template Literal)에 적용하고자 하는 CSS 스타일을 전달하여 컴포넌트를 생성한다. 변수명은 일반적인 React 컴포넌트와 마찬가지로 대문자로 시작해야 한다.
...
전달받은 props는 `sytled-components` 컴포넌트 생성 메서드의 백틱 내부에서 사용 가능하다. 템플릿 리터럴로 함수를 전달하고, 함수의 매개변수로 props를 전달받아 동적인 스타일 지정할 수 있다. 
...
CSS Module은 리액트 프로젝트에서 컴포넌트를 스타일링 할 때 CSS 선택자가 중첩되는 것을 방지할 수 있는 기능이다(주로 클래스 사용).

CSS Module 파일이 import되면 CSS 파일에 선택자로 지정한 이름(클래스 혹은 id)을 `[filename]_[classname]__[hash]` 형태의 고유한 이름으로 변경하여 중복을 방지한다(서로 다른 CSS 파일 내에 동일한 선택자도 고유한 별개의 값이 된다). 

CSS Module은 이 기능은 `webpack`에서 사용하는 `css-loader`에서 지원되는데 `create-react-app`으로 생성한 프로젝트에선 별다른 추가 다운로드없이 사용 가능하다.

사용법은 간단하다. CSS 파일의 확장자 명을 `name.module.css` 형태로 설정하고, 적용할 파일에 다음과 같이 이름을 설정하여 import 해주면 된다.

```
import [사용할 이름] from './Button.module.css'; 
```

그런 다음 스타일을 적용할 컴포넌트의 `className`에 불러온 CSS 모듈로부터 해당 클래스를 참조하여 적용시켜 준다.

CSS 모듈로 사용할 CSS 파일 내에서 클래스 선택자로 지정한 클래스 명이 곧 객체의 프로퍼티처럼 참조할 수 있는 값이 된다.
...
CSS Module을 사용하면 별다른 네이밍 규칙 없이도 CSS 클래스를 중복되지 않게 작성하기 쉽다.
