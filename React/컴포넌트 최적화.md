...
`useMemo`의 `memo`는 메모이제이션(memoization)을 의미한다. 자주 사용되는 데이터의 경우 `useMemo`로 메모리에 캐시하여 리렌더링 시 불필요하게 발생할 수 있는 연산을 건너뛴다.
...
두 번째로 의존성 배열을 전달한다. 이 의존성이 변경되었을 때만 첫 번째로 전달한 콜백을 호출하고 콜백이 반환한 새 값을 또 다시 새롭게 메모이제이션 한다(콜백 내부에서 사용되는 데이터 중, 리렌더링으로 인해 변경될 여지가 있는 데이터는 모두 의존성에 전달 해야한다).
...
리액트는 성능이 매우 좋기 때문에, 일반적인 경우 렌더링 횟수에 대해서 큰 고민을 하지 않아도 된다. 하지만 렌더링이 너무 지나치고, 또 컴포넌트가 무거운 작업을 수행하기까지 한다면 성능에 영향을 끼칠 수 있기 때문에 최적화를 해줄 필요가 있다.

`React.memo`는 이런 경우에 유용하게 사용할 수 있는 리액트 API이다. `React.memo`를 사용하면 자식 컴포넌트를 메모이제이션 하여 부모 컴포넌트가 리렌더링 되어도, 부모 컴포넌트로부터 전달받은 `props`가 변경되지 않는 경우 렌더링을 스킵한다. 
...
```
<!--  내용 수정 -->
첫 번째 인수로 메모이제이션 할 컴포넌트를 전달하고, 두 번째 인수에는 컴포넌트의 이전 `props`와 새 `props`를 전달받는 함수를 받는다. -> 일반적으로 사용하지 않으며 기본적으로 리액트가 이전 `props`와 새 `props`를 `Object.is` 메서드로 비교한다.
