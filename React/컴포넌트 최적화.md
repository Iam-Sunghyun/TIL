...
리액트는 성능이 매우 좋기 때문에, 일반적인 경우 렌더링 횟수에 대해서 큰 고민을 하지 않아도 된다. 하지만 렌더링이 너무 지나치고, 또 컴포넌트가 무거운 작업을 수행하기까지 한다면 성능에 영향을 끼칠 수 있기 때문에 이런 경우 최적화가 필요할 수 있다.
...
첫 번째 인수로 메모이제이션 할 컴포넌트를 전달하고, 두 번째 인수에는 컴포넌트의 이전 `props`와 새 `props`를 전달받는 함수를 받는다. -> 두 번째 인수는 일반적으로 사용하지 않으며 전달하지 않으면 기본 값으로 리액트가 `Object.is` 메서드로 이전 `props`와 새 `props`를 비교한다.
