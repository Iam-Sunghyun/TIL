...
API 슬라이스 엔드포인트에 정의한 mutation 함수로 제공된 인수와 함께 서버에 mutation을 요청하며 `Promise`를 반환한다.
...
`useMutation` 훅이 반환하는 트리거 함수를 호출하면 `Promise`를 반환 하는데 성공/실패와 관계없이 응답 데이터와 메타 정보를 포함하는 객체를 반환한다.

| 상태                   | Promise 결과                     | 반환 객체 주요 내용                                                                       |
| :--------------------- | :------------------------------- | :---------------------------------------------------------------------------------------- |
| **성공 시 (Resolved)** | `Promise<{ data: T, meta: M }>`  | 서버에서 받은 응답 데이터 (`data`)와 요청/응답 관련 메타 정보 (`meta`)를 포함한다.        |
| **실패 시 (Rejected)** | `Promise<{ error: E, meta: M }>` | 요청 실패의 원인이 된 에러 객체 (`error`)와 요청/응답 관련 메타 정보 (`meta`)를 포함한다. |

</br>

### 2. `.unwrap()` 사용 시 반환 값 구조 (권장)

`Promise` 체인에 `.unwrap()`을 사용하면, 응답 처리 방식이 간결해지고 순수한 데이터만 얻을 수 있다.
...
추가로 mutation 요청 상태가 변경되면 컴포넌트 리렌더링이 일어난다.
