...
이렇게 불변성 유지를 위해 매번 새 객체를 생성하고 할당하는 것이 성능에 문제가 될 것 같지만 상태 변경 여부를 쉽게 판단할 수 있기 때문에 프로퍼티를 대조하며 비교하는 것보다 훨씬 성능 면에서 유리하다고 한다.
...
같은 입력에 대해 결과가 항상 일정 해야 테스트를 할 수 있는데 리듀서가 순수 하지 않다면, 예를 들어 `fetch`, `Date.now()`와 같은 API가 있다면 테스트 결과가 매번 달라 질 수 있다. 이러한 코드는 애플리케이션이 커짐에 따라 복잡성이 증가하고 예측하기 어렵게 만들어 유지 보수에 좋지 않다
